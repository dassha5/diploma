...
with st.sidebar:
    ...
    st.info("У налаштуваннях Streamlit можна змінити тему інтерфейсу на світлу або темну.")
...
with col2:
    ...
    if st.button("🔊 Озвучити результат"):
        if 'last_detected' in st.session_state:
            speak_text(st.session_state.last_detected)
        else:
        ...
        if results.multi_hand_landmarks:
            ...
            if len(features) == 63:
                ...
                if prediction_count >= FRAME_THRESHOLD:
                    current_display = ukr_labels.get(prediction, prediction)
                    st.session_state.last_detected = current_display

        if current_display == "Руку не знайдено":
            result_placeholder.markdown(
                f"<p class='big-font' style='color: grey; font-size: 40px;'>{current_display}</p>",
                unsafe_allow_html=True
            )
        elif current_display == "Розпізнавання...":
            result_placeholder.markdown(
                f"<p class='big-font' style='color: orange; font-size: 40px;'>{current_display}</p>",
                unsafe_allow_html=True
            )
        else:
            result_placeholder.markdown(
                f"<p class='big-font'>{current_display}</p>",
                unsafe_allow_html=True
            )
